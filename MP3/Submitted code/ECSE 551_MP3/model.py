...
class MyNet(nn.Module):
    ...
    def __init__(
        ...
        self.conv1_seq = nn.Sequential(conv1, act_fn)
        ...
        self.conv2_seq = nn.Sequential(conv2, act_fn, nn.MaxPool2d(kernel_size=(2, 2)))
        ...
        self.conv3_seq = nn.Sequential(conv3, act_fn, nn.MaxPool2d(kernel_size=(2, 2)))
        ...
        self.fc1_seq = nn.Sequential(self.fc1, act_fn, nn.Dropout(p=dropout_prob))

        in_size = out_size
        out_size = 64
        fc2 = nn.Linear(in_size, out_size)
        fc2_drop = dropout_rate
        self.fc2_seq = nn.Sequential(fc2, act_fn, nn.Dropout(p=dropout_prob))
...
def get_model(
    model_type: Literal["MyNet", "LeNet5", "VGG13", "VGG16"],
    act_fn,
    dropout_prob,
    img_size,
):
